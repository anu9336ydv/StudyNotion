...
const {
    ...
    DELETE_ALL_COURSE_API
} =coursesEndpoint


export async function getIntructorCourses(token){
    ...
    try{
       ...
       // console.log("response of call ",response)
        ...
        const timeDuration = response.data.timeDuration

    }
    catch(error){
        console.log("error in getting enrolled courses >>> ",error)
        toast.error("Could Not  Courses")
...
export const deleteAllCourse = async (token) => {
    const toastId = toast.loading("Loading...")
    try {
      const response = await apiConnector("DELETE", DELETE_ALL_COURSE_API,null, {
        Authorization: `Bearer ${token}`,
      })
      ...
      console.log("DELETE All COURSEs API ERROR............", error)
      toast.error(error.message)
    }
    toast.dismiss(toastId)
  }
